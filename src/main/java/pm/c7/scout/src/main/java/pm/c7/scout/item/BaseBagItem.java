...
public class BaseBagItem extends TrinketItem {
    ...
    public boolean canEquip(ItemStack stack, SlotReference slot, LivingEntity entity) {
        ...
        ItemStack equippedStack = ItemStack.EMPTY;
        try {
            Inventory inv = slot.inventory();
            int index = slot.index();

            if (inv != null && index >= 0 && index < inv.size()) {
                equippedStack = inv.getStack(index);
            }
        } catch (Throwable ignored) {
        }

        // fix del bug de creativo:
        // nunca permitir reemplazar directamente una bag ya equipada con otra
        ...
        if (this.type == BagType.POUCH) {
            boolean leftEmpty = ScoutUtil.findBagItem(player, BagType.POUCH, false).isEmpty();
            boolean rightEmpty = ScoutUtil.findBagItem(player, BagType.POUCH, true).isEmpty();
